...
describe('TorrentStore', () => {
  describe('filteredTorrents', () => {
    ...
    // TODO: Review those specs
    describe('when showing waiting for downloading', () => {
      it('should only show waiting downloading torrents', () => {
        store.statusFilter = STATUS_DOWNLOAD_WAIT;

        expect(mapStatus(store.filteredTorrents)).toContain(STATUS_DOWNLOAD_WAIT);
        expect(store.filteredTorrents.length).toBe(1);
      });
    });
    describe('when showing waiting for checking', () => {
      it('should only show checking torrents', () => {
        store.statusFilter = STATUS_CHECK_WAIT;

        expect(mapStatus(store.filteredTorrents)).toContain(STATUS_CHECK_WAIT);
        expect(store.filteredTorrents.length).toBe(1);
      });
    });
    describe('when showing waiting for seeding', () => {
      it('should only show waiting seeding torrents', () => {
        store.statusFilter = STATUS_SEED_WAIT;

        expect(store.filteredTorrents[0].status).toBe(STATUS_SEED_WAIT);
        expect(store.filteredTorrents.length).toBe(1);
      });
    });
    describe('when showing checked', () => {
      it('should only show checked torrents', () => {
        store.statusFilter = STATUS_CHECK;

        expect(store.filteredTorrents[0].status).toBe(STATUS_CHECK);
        expect(store.filteredTorrents.length).toBe(1);
      });
    });
